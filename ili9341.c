...
#define spi_begin() PORTB &= ~_BV(PB4) // CS low
#define spi_end() PORTB |= _BV(PB4) // CS high
...
void ili9341_init(void) {
	PORTB |= _BV(PB1); // set RST high for normal operation
	DDRB |= _BV(PB1) | _BV(PB0) | _BV(PB4); // RST, DC and CS as output
	// Initialize SPI
	DDRB |= _BV(PB5) | _BV(PB7); // MOSI and SCK as output
	SPCR = _BV(SPE) | _BV(MSTR); // Mode 0, fOsc/4
	SPSR |= _BV(SPI2X); // Double clock rate
	PORTB |= _BV(PB4); // CS low during startup
	// Hardware reset
	PORTB &= ~_BV(PB1);
	_delay_ms(5);
	PORTB |= _BV(PB1);
